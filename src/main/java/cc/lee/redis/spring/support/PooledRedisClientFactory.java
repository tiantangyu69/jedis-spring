 ...
 */
public class PooledRedisClientFactory implements InitializingBean {
	private String addr;
	...
	private Integer db;
	private static RedisClientConfig redisClientConfig = new RedisClientConfig();

	RedisClient create() throws Exception {
		return createCommands(redisClientConfig);
	}

	static RedisClient createCommands(RedisClientConfig config)
			throws Exception {
